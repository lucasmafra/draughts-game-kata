...
describe('Board', () => {
    ...
    it('should have a dark square at the lower-leftmost square', () => {
        const LOWER_ROW = 1
        const LEFT_COL = 1
        const square = board.squareAt(pos(LOWER_ROW, LEFT_COL))
        expect(square.color).toBe(ColorEnum.DARK)
    })

    it('should alternate squares color between light and dark', () => {
        expect(board.squareAt(pos(1, 1)).color).toBe(ColorEnum.DARK)
        expect(board.squareAt(pos(1, 2)).color).toBe(ColorEnum.LIGHT)
        expect(board.squareAt(pos(2, 1)).color).toBe(ColorEnum.LIGHT)
    ...
    it('no light square can contain any piece', () => {
        piecesAt(
            ...
            pos(5,2),  pos(5,4),  pos(5,6),  pos(5,8),  pos(5,10),
            pos(6,1),  pos(6,3),  pos(6,5),  pos(6,7),  pos(6,9),
    ...
    it('should initialize 20 light pieces in the first 4 rows', () => {
        piecesAt(
    ...
    it('should initialize 20 dark pieces in the the last 4 rows', () => {
        piecesAt(
    ...
    it('should move a piece from one square to another', () => {
        board.move(pos(4,2), pos(5,3))
        expect(board.squareAt(pos(4,2)).piece).toEqual(undefined)
        expect(board.squareAt(pos(5,3)).piece).toEqual(expect.anything())
    })
